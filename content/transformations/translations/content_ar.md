...
بمرور الوقت ، قمنا بتقليد تناظر الطبيعة في الفن والهندسة المعمارية والتكنولوجيا والتصميم. يبدو أن الأشكال والأنماط المتماثلة تبدو أكثر جمالًا _<<<<_ من الأشكال غير المتماثلة.
...
[__التحول الصلب__](gloss:rigid-transformation) هو نوع خاص من التحول لا يغير حجم الشكل الأصلي وشكله. تخيل أنها مصنوعة من مادة صلبة مثل الخشب أو المعدن: يمكننا تحريكها ، وتحويلها وقلبها ، ولكن لا يمكننا التمدد أو تشويهها.
...
بالنسبة للتحويلات الصلبة ، تكون الصورة دائمًا [[متوافقة مع|the same as|opposite to]] الصورة الأصلية. هناك ثلاثة أنواع مختلفة من التحولات الجامدة:
...
{.text-center} التحول الذي {{}} ينقل_ الشكل ببساطة يسمى [__الترجمة__](gloss:translation).
...
{.text-center} التحول الذي _يدور_ الشكل يسمى [__دوران__](gloss:rotation).
...
[__الترجمة__](gloss:translation) هي تحول ينقل كل نقطة من الشكل بنفس المسافة في نفس الاتجاه.
...
{.caption} تمت الترجمة بواسطة ([[-4]] ، [[2]])
...
الآن حان دورك - ترجمة الأشكال التالية كما هو موضح:
...
{.caption} ترجمة بواسطة (3 ، 1) _{span.check(when="drag-0")}_
...
{.caption} الترجمة من خلال (–4 ، –2) _{span.check(when="drag-1")}_
...
{.caption} ترجمة بواسطة (5، –1) _{span.check(when="drag-2")}_
...
الانعكاس [__<<<<__](gloss:reflection) هو تحول "يقلب" أو "يعكس" شكلاً عبر خط. يُسمى هذا الخط __خط الانعكاس__.
...
لاحظ أنه إذا كانت نقطة ما تقع على خط الانعكاس ، فستكون صورتها [[مثل|smaller than|opposite to]] النقطة الأصلية.
...
{.r} لعكس هذا الشكل عبر [خط الانعكاس](target:refl) ، علينا أن نعكس كل [قمة](gloss:polygon-vertex) بشكل فردي ثم نربطها مرة أخرى. _{button.next-step} متابعة_
...
{.r.reveal(when="next-1")} باستخدام [منقلة](target:protractor) ، يمكننا قياس [زاوية ${ang*10} °](target:angle) حول مركز الدوران. فلنرسم [خط ثانٍ](target:l2) عند تلك الزاوية. _{button.next-step} متابعة_
...
تعتبر التحولات مفهومًا مهمًا في العديد من أجزاء الرياضيات ، وليس الهندسة فقط. على سبيل المثال ، يمكنك تحويل [_وظائف_](gloss:function) عن طريق تحويل أو تدوير [الرسوم البيانية](gloss:function-graph). يمكنك أيضًا استخدام التحويلات لتحديد ما إذا كان الشكلان متطابقان [<<<<](gloss:congruent).
...
[__التناظر__](gloss:symmetry) في كل مكان حولنا ، ومفهوم بديهي: تبدو الأجزاء المختلفة من الكائن _متشابهة_ بطريقة ما. ولكن باستخدام التحولات ، يمكننا تقديم تعريف رياضي أكثر دقة لما يعنيه التماثل _حقًا_:
...
{.text-center} يمكننا أن نعكس هذه الفراشة ، وتبدو كما هي بعد ذلك. نقول أنه يحتوي على __تناظر انعكاسي__.
...
{.text-center} يمكننا تدوير هذه الزهرة ، وتبدو كما هي بعد ذلك. نقول أنه يحتوي على __تناظر دوراني__.
...
الشكل له [__تناظر انعكاسي__](gloss:rotational-symmetry) إذا كان نفس الشكل بعد الانعكاس. ويسمى خط الانعكاس [__محور التناظر__](gloss:axis-of-symmetry) ، ويقسم الشكل إلى نصفين [[متطابقين|equal|similar]]. يمكن أن يكون لبعض الأشكال أيضًا أكثر من محور واحد للتماثل.
...
{.caption} يحتوي هذا الشكل على [[2]] محور تناظر.
...
{.caption} يحتوي المربع على [[4]] محاور تناظر.
...
إليك بعض الأشكال الأخرى. أكملها بحيث يكون لها تناظر انعكاسي:
...
يمكن أن يكون للأشكال والحروف والصور تناظر انعكاسي ، ولكن يمكن أن يكون للأرقام والكلمات والجمل بأكملها!
...
إذا تجاهلنا المسافات وعلامات الترقيم ، فإن الجمل القصيرة أدناه أيضًا لها تناظر انعكاسي. هل يمكنك الخروج بنفسك؟

{.text-center} ليس غريبًا أو حتى.  
الجوز [[<<<<]] لجرة من التونة.  
يا موز [[ولد]]!
...
للشكل [__تناظر دوراني__](gloss:rotational-symmetry) إذا كان متشابهًا بعد التدوير (بأقل من 360 درجة). عادةً ما يكون مركز الدوران [<<<<](gloss:center-of-rotation) في منتصف الشكل فقط. [__ترتيب التناظر__](gloss:order-of-symmetry) هو عدد الاتجاهات المتميزة التي يبدو الشكل فيها متشابهًا. يمكنك أيضًا التفكير في الأمر باعتباره _عدد المرات التي يمكننا فيها تدوير الشكل_ ، قبل أن نعود إلى البداية. على سبيل المثال ، يحتوي هذا الثلج على الطلب [[6]].
...
{.caption} طلب [[2]] ، زاوية [[180]] °

::: column(width=220)

    img(src="images/flower.jpg" width=200 height=200)

{.caption} طلب [[8]] ، الزاوية [[45]] °
...
الآن أكمل هذه الأشكال ، بحيث يكون لها تناظر دوراني:
...
>بعض الأشكال لها أكثر من تناظر واحد - فلنلق نظرة على [مربع](gloss:square) كمثال بسيط.
...
{.reveal(when="blank-0")} كما أن لديها تناظر دوراني بمقدار [[90]] ° و [[180]] ° و [[270]] °.

{.reveal(when="blank-1 blank-2 blank-3")} وأخيرًا ، يمكننا التفكير في "عدم فعل أي شيء" كنوع خاص آخر من التناظر - لأن النتيجة (من الواضح) هي نفسها كما كانت من قبل. يُسمى هذا أحيانًا __الهوية__.
...
اقض بعض الوقت في اللعب باستخدام الآلة الحاسبة للتماثل ، وحاول العثور على أي أنماط. هل يمكنك إكمال هذه الملاحظات؟ * تؤدي إضافة دورتين إلى منح [[دورانًا|a reflection]] (أو الهوية). * ستؤدي إضافة انعكاسين دائمًا إلى [[دوران|a reflection]] (أو الهوية). * تؤدي إضافة نفس التماثلات بالترتيب المعاكس [[أحيانًا إلى نتيجة|always gives a different|always gives the same]] مختلفة. * لا تؤدي إضافة الهوية [[إلى فعل أي شيء|returns a reflection|returns the opposite]].

---
> id: group-axioms

ربما أدركت بالفعل أن إضافة __{.orange} تناظرات__ تشبه إلى حد كبير إضافة __{.green} أعداد صحيحة__:
...
في [الأقسام السابقة](/course/transformations/symmetry) شاهدنا نوعين مختلفين من التناظر يتوافقان مع تحولين مختلفين: التدوير والانعكاسات. ولكن هناك أيضًا تناظر للنوع الثالث من التحول الجامد: [[الترجمات|spins|flips]].
...
يمكن أن يحتوي النمط على أكثر من نوع واحد من التناظر. ومثلما هو الحال بالنسبة للمربعات ، يمكننا العثور على [مجموعة تناظر](gloss:symmetry-group) لنمط يحتوي على جميع أوجه التماثل المختلفة.

لا تخبرك هذه المجموعات كثيرًا عن كيفية ظهور النمط _<<<<_ (مثل ألوانه وأشكاله) ، فقط كيف _يتكرر_. يمكن أن يكون للعديد من الأنماط المختلفة نفس مجموعة التماثل - طالما يتم ترتيبها وتكرارها بنفس الطريقة.
...
في عام 1915 ، لاحظ عالم الرياضيات الألماني [Emmy Noether](bio:noether) أن شيئًا مشابهًا ينطبق على [قوانين الطبيعة](gloss:laws-of-nature).

::: column.grow

على سبيل المثال ، تخبرنا تجربتنا أن قوانين الفيزياء هي نفسها في كل مكان في الكون. لا يهم إذا أجريت تجربة في لندن ، أو في نيويورك ، أو على كوكب المريخ - يجب أن تكون قوانين الفيزياء هي نفسها دائمًا. بطريقة ما ، لديهم [[التناظر الانتقالي|reflectional symmetry]].

{.reveal(when="blank-0")} وبالمثل ، لا يهم إذا أجرينا تجربة أثناء مواجهة الشمال أو الجنوب أو الشرق أو الغرب: قوانين الطبيعة لها [[تناظر دوراني|glide reflection symmetry]].
...
قد تبدو هذه "التماثلات" في البداية عديمة المعنى تمامًا ، لكنها يمكن أن تخبرنا كثيرًا عن كوننا. تمكنت Emmy Noether من إثبات أن كل تناظر يتوافق مع كمية مادية معينة _محفوظة_.
...
لقد نظرنا حتى الآن للتو على تحويلات [[جامدة|congruent|visual]]. _{span.reveal(when="blank-0")} دعنا نفكر الآن في واحد غير: [__تمدد__](gloss:dilation) يغير حجم الشكل بجعله أكبر أو أصغر._
...
جميع التوسعات لها [__مركز__](target:center) و [__عامل قياس__](->.scale-target). المركز هو النقطة المرجعية لعامل التمدد والقياس يخبرنا كم يمتد الشكل أو يتقلص. إذا كان [عامل المقياس](gloss:scale-factor) بين 0 و 1 ، فإن الصورة [[أصغر|larger]] من الصورة الأصلية. إذا كان عامل المقياس أكبر من 1 ، فإن الصورة [[أكبر|smaller]] من الصورة الأصلية.
...
{.todo} يتوفر قريبًا - المزيد عن التوسعات    
