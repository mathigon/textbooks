...
Результат преобразования называется [__образ__](gloss:transformation-image). Образ фигуры `A` обычно обозначается `A'` (произносится как «А штрих»).

[Первый пример](->#star) выше является особенным, потому что он только перемещает и вращает исходную звезду, но не меняет ее размер или форму. Преобразования с этим свойством называются __изометрическими преобразованиями__.
...
{.text-center} Преобразование, которое просто __перемещает__ форму, называется [__параллельный перенос__](gloss:translation).
...
{.text-center} Преобразование, которое __отражает__ форму, называется [__осевая симметрия__](gloss:reflection).
...
{.text-center} Преобразование, которое __вращает__ форму, называется [__поворот__](gloss:rotation).
...
Мы также можем объединить несколько типов преобразований, чтобы создать более сложные, например, поворот с последующей осевой симметрией.
...
На координатной плоскости мы можем распознать параллельный перенос по тому, на сколько фигура перемещается вдоль оси _x_ и оси _y_. Например, перенос на (3, 5) перемещает фигуру на 3 клетки вдоль оси _x_ и на 5 вдоль оси _y_.
...
[__Осевая симметрия__](gloss:reflection) - это преобразование, которое «отражает» форму относительно прямой. Эта прямая называется __ось симметрии__.
...
Во всех приведенных выше примерах ось симметрии была горизонтальной, вертикальной или под углом 45 °, что облегчало рисование образов. Если это не так, то конструкция требует немного большей работы:
...
{.r} Чтобы отразить эту форму относительно [оси симметрии](target:refl), мы должны отразить каждую [вершину](gloss:polygon-vertex) по отдельности, а затем соединить их снова. _{button.next-step} Продолжить_
...
Попробуйте повернуть фигуры ниже вокруг красного центра поворота:
...
[__Симметрия__](gloss:symmetry) повсюду вокруг нас, и интуитивно понятная концепция: различные части объекта в некотором роде выглядят __одинаково__. Но используя преобразования, мы можем дать гораздо более точное математическое определение того, что же симметрия действительно означает:
...
{.text-center} Мы можем повернуть этот цветок относительно центра, и он будет выглядеть так же. Мы говорим, что он имеет __центральную симметрию__.
...
Вот еще несколько фигур. Завершите их так, чтобы они имели осевую симметрию:
...
Форма имеет [__центральную симметрию__](gloss:rotational-symmetry), если она выглядит одинаково после поворота (менее чем на 360°). [Центр вращения](gloss:center-of-rotation) обычно является просто серединой фигуры.
...
{.reveal(when="blank-1 blank-2 blank-3")} И, наконец, мы можем принять «бездействии» как особый тип симметрии - потому что результат (очевидно) такой же, как и раньше. Это иногда называют __тождеством__.
...
Всякий раз, когда вы складываете две симметрии квадрата, вы получаете новый. Вот «калькулятор симметрии», где вы можете попробовать сложение сами:
...
Вы, возможно, уже поняли, что сложение __{.orange} симметрий__ на самом деле очень похоже на сложение __{.green} целых чисел__:

    ol.proof
      li.r
        | Сложение двух #[strong.orange симметрий]/#[strong.green целых чисел] всегда дает другую #[strong.orange симметрию]/#[strong.green целое число]:
        .text-center.axiom 
      ...
      li.r.reveal(when="next-1")
        | Любая #[strong.orange симметрия]/#[strong.green целое число] имеет #[strong противоположную] #[strong.orange симметрию]/#[strong.green целое число] которое при сложении дает тождество или ноль:
        .text-center.axiom 
          img.cube(src="images/cube-1.svg" width=32 height=32)
          mo +
          img.cube(src="images/cube-3.svg" width=32 height=32)
...
В математика, любой набор, которая обладает этими свойствами, называется [__группа__](gloss:group). Некоторые группы (например, __{.orange} симметрии__ квадрата) имеют только конечное число элементов. Другие (например, __{.green} целых чисел__) бесконечны.
...
Группы появляются везде в математике. Элементы могут быть числами или симметриями, но также полиномами, перестановками, матрицами, функциями… все, что подчиняется трем правилам. Ключевая идея __теории групп__ состоит в том, что нас не интересуют отдельные элементы, только то, __как они взаимодействуют друг с другом__.

::: column.grow

Например, группы симметрии разных молекул могут помочь ученым предсказать и объяснить свойства материалов, состоящих из них. Группы также можно использовать для анализа выигрышной стратегии в настольных играх, поведения вирусов в медицине, различных гармоний в музыке и многих других концепций…
...
{.caption} Свойства молекулы CCl<sub>4</sub> (слева) и аденовируса (справа) определяется их симметрией.
...
В [предыдущих разделах](/course/transformations/symmetry) мы познакомились с двумя видами симметрии, соответствующих двум разным преобразованиям: поворот и отражение относительно оси. Но существует также симметрия для третьего вида изометрического преобразования: [[параллельного переноса|вращения|переворота]].

---
> id: wallpaper-groups-1
> goals: play-0 play-1

[__Симметрия переноса__](gloss:translational-symmetry) не работает для отдельных объектов, таких как цветы или бабочки, но она работает для регулярно повторяющихся узоров, которые простираются во всех направлениях:
...
Оказывается, что, хотя существует бесконечно много возможных вариантов узоров, все они имеют одну из 17 различных групп симметрии. Они называются __Группы орнаментов__. Каждая группа орнаментов определяется сочетанием осевых, центральных, скользящих и симметрий переноса. Сможете ли вы увидеть [центры симметрии](gloss:center-of-rotation) и [оси симметрии](gloss:axis-of-symmetry) в примерах ниже?
...
Группы орнаментов были о плоских, двумерных узорах. Мы можем сделать нечто подобное для трехмерных моделей: они называются кристаллографическими группами, и их 219!
