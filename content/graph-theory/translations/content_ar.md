...
في الرياضيات ، يمكن تمثيل كل هذه الأمثلة على أنها [__رسوم بيانية__](gloss:graph) (يجب عدم الخلط بينها وبين _رسم بياني_ للدالة). يتكون الرسم البياني من _نقاط_ معينة تسمى [[رؤوس|circles|crossings]] ، بعضها متصل بـ [[حواف|boundaries|pairs]].
...
يمكننا رسم تخطيط الرسوم البيانية البسيطة باستخدام الدوائر والخطوط. موضع القمم وطول الحواف غير ذي صلة - نحن نهتم فقط _بكيفية ارتباطهم_ ببعضهم البعض. يمكن أن تتقاطع الحواف مع بعضها البعض ، ولا يجب أن تكون مستقيمة.
...
{.caption} تتكون بعض الرسوم البيانية من مجموعات متعددة من القمم التي لا ترتبط ببعضها البعض بالحواف. هذه الرسوم البيانية __غير متصلة__.

::: column(width=200)

    svg.graph(height=120 width=200 style="margin: 0 auto .8em")

{.caption} قد تحتوي الرسوم البيانية الأخرى على حواف متعددة بين الأزواج نفسها من القمم ، أو القمم المتصلة بأنفسهم (الحلقات).
...
يمكننا إنشاء رسوم بيانية جديدة من الرسم البياني الحالي عن طريق إزالة بعض القمم والحواف. النتيجة تسمى [__subgraph__](gloss:subgraph). هنا يمكنك أن ترى بعض الأمثلة على الرسوم البيانية ، مع حواف ملونة ورؤوس تشير إلى رسم فرعي محتمل:
...
نقول أن [__الترتيب__](gloss:graph-order) للرسم البياني هو عدد القمم التي يحتويها. [__درجة__](gloss:graph-degree) من قمة الرأس هي عدد الحواف التي تلتقي في ذلك الرأس.
...
تسمى الرسوم البيانية التي تتكون من حلقة واحدة من القمم [__دورات__](gloss:graph-cycle). تحتوي جميع الدورات على [[نفس عدد الحواف والرؤوس|more edges than vertices|fewer edges than vertices]].
...
أولاً ، نحتاج إلى تحويل خرائط المدينة إلى رسوم بيانية ذات حواف ورؤوس. يتم تمثيل كل جزيرة أو منطقة من الأرض بواسطة [[قمة|an edge|an area]] وكل جسر يربط بين منطقتين يمثله [[حافة|vertex|street]] مقابلة.
...
تمامًا مثل خرائط المدينة من قبل ، نجد أن بعض الرسوم البيانية ممكنة بينما البعض الآخر ليس كذلك. لمساعدتنا على فهم السبب ، دعنا نصنف كل قمة {::: column(width=220 parent="padded-thin")}](gloss:graph-degree). ثم يمكننا تلوين القمم بطرق مختلفة ، ومحاولة الكشف عن نمط:
...
بمقارنة هذه الأرقام بالرسوم البيانية الممكنة والأخرى غير الممكنة ، يبدو أنه يمكن رسم الرسم البياني إذا كان [[لا يحتوي على أكثر من رأسين "فرديين"|only has “even” vertices|has no vertices with an order larger than 4|has an odd number of vertices|has no vertices of order 3]]. يمكن تفسير هذا الشرط إذا نظرنا إلى قمة واحدة فقط في الرسم البياني:
...
---
> id: handshakes-1
> section: handshakes ## المصافحة والتعارف

::: column.grow

لقد تمت دعوتك لحفلة عيد ميلاد رائعة مع أصدقائك. بما في ذلك أنت والمضيف ، هناك ${hnd}{hnd|5|3,15,1} شخصًا موجودًا. في المساء ، عندما يستعد الضيوف للمغادرة ، يصافح الجميع بعضهم البعض. كم عدد المصافحات في المجموع؟ يمكننا تمثيل المصافحات باستخدام رسم بياني: كل شخص هو [[قمة الرأس|an edge]] ، وكل مصافحة [[حافة|a vertex]].
...
للأسف ، هذه الإجابة ليست صحيحة تمامًا. لاحظ كيف <x-target to=".handshakes tr:first-child td:first-child, .handshakes tr:first-child td:nth-child(2)"> أول إدخالين في الصف العلوي </x-target> في الواقع هي نفسها ، فقط انقلبت. في الواقع ، لقد حسبنا كل مصافحة [[مرتين|once|three times]] ، _{span.reveal(when="blank-0")} مرة واحدة لكل من الأشخاص المعنيين. وهذا يعني أن العدد الصحيح للمصافحة لـ ${n}{n|5|2,25,1} ضيفًا هو `(var("n") × var("n-1"))/2 = var("n*(n-1)/2")`._

---
> id: handshakes-3

الرسوم البيانية للمصافحة خاصة لأن كل قمة متصلة بكل قمة أخرى. تسمى الرسوم البيانية بهذه الخاصية __الرسوم البيانية الكاملة__. غالبًا ما يتم اختصار الرسم البياني الكامل ذو 4 رؤوس كـ `K_4` ، ويعرف الرسم البياني الكامل الذي يحتوي على 5 رؤوس باسم `K_5` ، وهكذا. لقد أظهرنا للتو أن رسمًا بيانيًا كاملاً يحتوي على `n` رؤوس ، `K_n` ، له `(n × (n-1))/2` حواف.
...
في هذه الحالة ، يتكون الرسم البياني المقابل من مجموعتين منفصلتين من القمم. كل قمة متصلة بجميع القمم في مجموعة [[المقابلة|its own]] ، ولكن لا توجد أي قمة في مجموعة [[الخاصة بها|the opposite]]. تسمى الرسوم البيانية التي تحتوي على هذا التنسيق __الرسوم البيانية الثنائية__.
...
> section: planar-graphs ## مخططات مستوية
...
{.text-center} `K_4` [[مستوي|is not planar]].

::: column(width=200)

    svg.graph#planar-3(width=200 height=200 style="margin-bottom: .4em;")

{.text-center} `K_5` [[ليست مستوية|is planar]].
...
### صيغة أويلر جميع الرسوم البيانية المستوية تقسم الطائرة التي يتم رسمها إلى عدد من المناطق ، تسمى __الوجوه__.

::: column(width=200)

    include svg/euler-2.svg

{.text-center} [[6]] رؤوس <br> [[5]] وجوه <br> [[10]] الحواف <br> _{span.euler-sum} 11 رؤوس + وجوه_

::: column(width=200)

    include svg/euler-1.svg

{.text-center} [[8]] رؤوس <br> [[7]] الوجوه <br> [[14]] الحواف <br> _{span.euler-sum} 15 رأس + وجوه_

::: column(width=200)

    include svg/euler-3.svg

{.text-center} [[12]] رؤوس <br> [[13]] الوجوه <br> [[24]] الحواف <br> _{span.euler-sum} 25 قمة + وجوه_
...
عند مقارنة هذه الأرقام ، ستلاحظ أن عدد الحواف يكون دائمًا [[أقل|bigger|the same]] من عدد الوجوه بالإضافة إلى عدد القمم. بعبارة أخرى ، _{.b.blue} F_ + _{.b.green} V_ = _{.b.red} E_ + 1. تسمى هذه النتيجة __معادلة أويلر__ وسميت باسم نفس [عالم الرياضيات](bio:euler) الذي حل مشكلة Königsberg Bridges. لسوء الحظ ، هناك عدد لا نهائي من الرسوم البيانية ولا يمكننا التحقق من كل واحد لمعرفة ما إذا كانت معادلة أويلر تعمل. بدلاً من ذلك ، يمكننا محاولة العثور على [إثبات](gloss:proof) بسيط يعمل مع أي رسم بياني ...
...
يمكن إنشاء أي رسم بياني (محدود) من خلال البدء برأس واحد و إضافة المزيد من القمم واحدا تلو الآخر. لقد أظهرنا أنه ، بغض النظر عن الطريقة التي نضيف بها رؤوس جديدة ، فإن معادلة أويلر صالحة. لذلك فهي صالحة لجميع الرسوم البيانية. العملية التي استخدمناها تسمى __الحث الرياضي__. إنها تقنية مفيدة للغاية لإثبات النتائج في عدد لا نهائي من الحالات ، ببساطة عن طريق البدء بالحالة الأبسط ، وإظهار أن النتيجة تصمد في كل خطوة عند إنشاء حالات أكثر تعقيدًا.     .svg-block: include svg/dominoes.svg
...
{.caption} __إيكوساهيدرون__  
__{.blue} 20__ وجوه  
__{.green} 12__ الذروات  
__{.red} 30__ الحواف
...
{.caption} __اقتطاع Icosahedron__  
...
كما كان من قبل ، يمكننا تحويل خريطة مع البلدان والحدود إلى رسم بياني مستوي: يصبح كل بلد [[قمة|an edge|a face]] ، والبلدان التي [[تشترك في حد|have the same colour]] تواصل بحافة:
...
تم إحراز تقدم بسيط في مشكلة الألوان الأربعة حتى عام 1976 ، عندما استخدم [Wolfgang Haken](bio:haken) و [Kenneth Appel](bio:appel) جهاز كمبيوتر لحلها أخيرًا. قاموا بتخفيض عدد لا حصر له من الخرائط الممكنة إلى 1936 حالة خاصة ، تم فحص كل منها بواسطة كمبيوتر استغرق أكثر من 1000 ساعة في المجموع.
...
دعونا نفكر ، مرة أخرى ، في الشبكات والخرائط. تخيل أن على خدمة التوصيل زيارة ${tsn}{tsn|8|2,50,1} مدن مختلفة لتوزيع الطرود. يمكننا التفكير في هذه المدن على أنها القمم في الرسم البياني. إذا كانت جميع المدن متصلة بالطرق ، فهذا [[رسم بياني كامل|cycle|bipartite graph]] ، لذلك هناك <mfrac> <mrow> ${tsn} × (${tsn} - 1) </mrow> <mn> 2 </mn> </mfrac> = ${tsn*(tsn-1)/2} إجمالي الحواف.
...
هناك احتمالات مختلفة لا تعد ولا تحصى لدورات هاميلتون في الرسوم البيانية الكاملة. في الواقع ، يمكننا اختيار أي قمة كبداية ، ثم اختيار أي من المدن المتبقية بأي ترتيب:
...
هذا يعني أنه يوجد إجمالي ${tsnPaths(tsn1)} مسارًا ممكنًا. الاختصار لهذا المنتج هو ${tsn1}! أو ${tsn1} __Factorial__.
...
الإنترنت ، على سبيل المثال ، هو رسم بياني افتراضي واسع. كل قمة هي صفحة ويب فردية ، وكل حافة تعني وجود ارتباط تشعبي بين صفحتين. لاحظ أن الروابط تسير باتجاه واحد فقط ، لذا فإن هذا الرسم البياني [[موجه|multi-line|conected]] ، وأن هذا الرسم البياني _كبير جدًا جدًا وكبير_.
...
أخيرًا ، دعونا نفكر في أحد الأمثلة الجيدة بشكل خاص على الرسوم البيانية الموجودة في الحياة اليومية: وسائل التواصل الاجتماعي. هنا ، تمثل القمم [[شخصًا|friends|networks]] وتمثل الحواف الصداقات أو الإعجابات أو الاشتراكات أو المتابعين.
