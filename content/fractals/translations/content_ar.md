...
{.caption} يتكون __Romanesco broccoli__ من [[مخاريط|cubes|spheres]] أصغر تدور حول واحدة أكبر.
...
صورة النمطي هندسي متكرر أخرى هي [__Sierpinski triangle__](gloss:sierpinski-triangle). في هذه الحالة ، نبدأ بمثلث كبير متساوي الأضلاع ، ثم نقطع بشكل متكرر المثلثات الأصغر من الأجزاء المتبقية.
...
تبدو النباتات في بداية هذا الفصل _<<<<_ تمامًا مثل الفركتلات ، ولكن من الواضح أنه من المستحيل إنشاء فركتلات _حقيقية_ في الحياة الواقعية. إذا واصلنا تكرار نفس النمط مرارًا وتكرارًا ، أصغر وأصغر ، فسنصل في النهاية إلى الخلايا أو الجزيئات أو الذرات التي لم يعد من الممكن تقسيمها.
...
الآن دعونا نلقي نظرة على مثلث Sierpinski. إذا قمنا بقياسها بعامل 2 ، يمكنك أن ترى أن "المساحة" تزداد بعامل [[3]].

{.reveal(when="blank-0")} فلنفترض أن _d_ هو بُعد مثلث Sierpinski. باستخدام نفس النمط أعلاه ، نحصل على `2^d = 3`. بمعنى آخر ، _يوم_ = [[`log_2(3)`|`log_3(2)`]] _{span.reveal(when="blank-1")} ≈ 1.585…_
...
مع كل تكرار ، نزيل بعض مساحة مثلث Sierpinski. إذا تمكنا من فعل ذلك بشكل لا نهائي عدة مرات ، فلن تكون هناك مساحة متبقية في الواقع: لهذا السبب فإن مثلث Sierpinski هو شيء بين منطقة ثنائية الأبعاد وخط أحادي البعد.
...
{.reveal(when="blank-0")} يسمى الشكل الناتج [__كوتش ندفة الثلج__](gloss:koch-snowflake) ، والتي تحمل اسم عالم الرياضيات السويدي [هيلج فون كوخ](bio:koch). لاحظ مرة أخرى أن [أجزاء صغيرة](target:t2) من حافة ندفة الثلج تبدو تمامًا مثل [أقسام أكبر](target:t1).
...
عندما نقوم بقياس جزء حافة واحدة من Koch Snowflake بمعامل 3 ، يبلغ طوله [[أربعة أضعاف|triples|doubles]].

{.reveal(when="blank-0")} باستخدام نفس العلاقة بين الأبعاد وعوامل القياس الموضحة أعلاه ، نحصل على المعادلة [[`3^d=4`|`2^d=4`|`2^d=3`|`4^d=3`]]. _{span.reveal(when="blank-1")} وهذا يعني أن بُعد Koch Snowflake هو `§d = log_3(4) ≈ 1.262`._
...
يشبه إنشاء رقائق Koch الثلجية [تسلسلًا تعاوديًا](gloss:sequence-recursive): فنحن نعرف شكل البداية (مثلث) ونعرف كيفية الانتقال من مصطلح إلى آخر (بإضافة المزيد من المثلثات على كل حافة):
...
{.reveal(when="blank-3 blank-4")} لنفترض أن [المثلث الأول](->#koch-0) تبلغ مساحته 1. ثم تبلغ المساحة الإجمالية لـ [المثلثات الثلاثة التالية](->#koch-1) هي `3 × 1/9 = 1/3`. تشكل جميع الخطوات التالية [[سلسلة هندسية|arithmetic series|quadratic series]] ، _{span.reveal(when="blank-5")} بنسبة مشتركة [[`4/9`|`9/4`|`4/3`]]._
...
يمكننا أيضًا محاولة حساب محيط ندفة الثلج Koch. كما رأينا من قبل ، يتغير طول المحيط بعامل [[`4/3`|`3/4`|`1/4`]] في كل خطوة.

{.reveal(when="blank-8")} هذا يعني أنه لدينا مرة أخرى سلسلة هندسية - ولكن في هذه الحالة [[لا تتقارب|converges to 0|doesn’t have a first term]]. _{span.reveal(when="blank-9")} وهذا يعني أن محيط ندفة الثلج Koch هو في الواقع __طويل بلا حدود__!_
...
{.reveal(when="blank-0")} يتكون المكعب `3×3×3` من 27 مكعبًا أصغر حجمًا ، ولكن هنا أزلنا بعضًا منها. تتكون إسفنجة Menger من [[20]] نسخة من نفسها ، وهي أصغر 3 مرات.

{.reveal(when="blank-1")} الآن يمكننا محاولة حساب البُعد _d_ من الإسفنج Menger تمامًا كما فعلنا مع Koch snowflake أعلاه. في هذه الحالة نحصل على `3^d = 20` أو `§d = log_3(20) ≈ 2.727`.
...
{.reveal(when="var-0")} يمكننا فقط الاستمرار مع الحكام الأصغر والأصغر ، وفي كل مرة ستصبح نتيجتنا على طول الخط الساحلي أطول قليلاً. تمامًا مثل Koch Snowflake من قبل ، يبدو أن الساحل البريطاني طويل جدًا! غالبًا ما يطلق عليه __مفارقة الخط الساحلي__. [متابعة](btn:next)
...
بعد عقود قليلة ، تعثر عالم الرياضيات [Benoit Mandelbrot](bio:mandelbrot) في عمل ريتشاردسون في كتاب مكتبة مهمل ، أثناء عمله في IBM. لقد أدرك أهميته ، وكذلك كيفية ارتباطه بأبحاث أحدث حول الفركتلات والأبعاد.
...
### المزيد من الصور النمطي هندسي متكرر في الطبيعة والتكنولوجيا
...
أحد الفركتلات التي رأيناها في الفصل السابق كان [__Sierpinski triangle__](gloss:sierpinski-triangle) ، والذي سمي على اسم عالم الرياضيات البولندي [Wacław Sierpiński](bio:sierpinski). يمكن إنشاؤه من خلال البدء بمثلث كبير متساوي الأضلاع ، ثم قطع المثلثات الأصغر من مركزه بشكل متكرر.

{.r.reveal(when="slider-0")} Wacław Sierpi theski كان أول علماء الرياضيات يفكرون في خصائص هذا المثلث ، لكنه ظهر قبل ذلك بقرون عديدة في الأعمال الفنية والأنماط والفسيفساء.
...
كما اتضح ، يظهر مثلث Sierpinski في مجموعة واسعة من المجالات الأخرى للرياضيات ، وهناك العديد من الطرق المختلفة لتوليدها. في هذا الفصل ، سوف نستكشف بعضها! [متابعة](btn:next)
...
قد تتذكر بالفعل مثلث Sierpinski من فصلنا على [__مثلث باسكال__](gloss:pascals-triangle). هذا هرم عدد فيه كل رقم هو مجموع الرقمين أعلاه. انقر على جميع الأرقام _حتى_ في المثلث أدناه ، لتمييزها - ومعرفة ما إذا لاحظت نمطًا:
...
يمكن أن يستمر مثلث باسكال إلى أسفل إلى الأبد ، وسيستمر نمط Sierpinski بمثلثات أكبر وأكبر. يمكنك بالفعل رؤية بداية مثلث أكبر ، بدءًا من الصف 16.
...
{.reveal(when="s1 s2 play")} هل اكتشفت [سجادة Sierpinski](action:carpet()) أو [ندفة الثلج الخماسية](action:snowflake()) بناءً على [__النسبة الذهبية__](gloss:golden-ratio)؟
...
هناك العديد من المتغيرات لمثلث Sierpinski ، وبعض الفركتلات الأخرى التي لها خصائص مشابهة وعمليات الإنشاء. تبدو بعضها ثنائية الأبعاد ، مثل _Sierpinski Carpet_ التي رأيتها أعلاه. البعض الآخر يبدو ثلاثي الأبعاد ، مثل هذه الأمثلة:
...
إذا كان `x_0 > 1` ، فإن التسلسل [[يختلف عن 535}: _{span.reveal(when="blank-0")} يستمر في النمو ، حتى اللانهاية._

::: column.sequence-cell(width=180)

إذا كان `x_0` بين –1 و 1 ، فإن التسلسل [[يتقارب|diverges]].

::: column.sequence-cell(width=180)

إذا كان `x_0 < -1` ، فإن التسلسل [[يختلف عن 541}.
...
{.reveal(when="wipe-a wipe-b wipe-c wipe-d")} كما ترى ، يتقارب التسلسل طالما أن `pill(x_0,"yellow","x0")` تقع [[داخل دائرة الوحدة| outside the unit square|above the _>>>>x<<<<_-axis]] _{span.reveal(when="blank-0")} (الدائرة ذات نصف القطر 1 ، متمركزة في الأصل)._

---

> id: julia-1

الآن دعونا نجعل الأمور أكثر صعوبة. بدلاً من مجرد تربيع الرقم السابق ، نضيف أيضًا ثابتًا _{.pill.red} ج_ في كل مرة (والذي يمكن أن يكون أي رقم مركب). وبعبارة أخرى ، `§x_n = x_(n-1)^2 + c`. هل تعتقد أننا ما زلنا نحصل على دائرة التقارب؟ ما الأشكال الأخرى التي تعتقد أننا قد نراها؟ [متابعة](btn:next)
...
عند إنشاء مجموعات جوليا المختلفة ، ربما لاحظت وجود بعض قيم _ج_ التي يختلف فيها كل تسلسل ، ويظل المستوى المعقد بأكمله أبيض. بعد عقود قليلة من جوليا و فاتو ، حاول جيل جديد من علماء الرياضيات رسم خريطة لهذه المناطق.
...
يسمى هذا الفراكتل [__Mandelbrot Set__](gloss:mandelbrot-set) ، وعندما يدور بزاوية 90 درجة ، يبدو تقريبًا مثل الشخص ، مع الرأس والجسم والذراعين. تم تعريفه ورسمه لأول مرة في عام 1978 ، في ورقة بحثية من قبل علماء الرياضيات روبرت بروكس وبيتر ماتلسكي:
...
مثل جميع صور النمطي هندسي متكرر ، يمكننا "تكبير" مجموعة ماندلبروت إلى الأبد ، وإيجاد أنماط جديدة على كل مقياس. هنا يمكنك تكبير جزء من مجموعة Mandelbrot يسمى __Seahorse Valley__. النقاط السوداء هي _داخل_ مجموعة Mandelbrot ، حيث يكون التسلسل مقيدًا. النقاط الملونة هي _خارج_ مجموعة ماندلبروت ، حيث يتباعد التسلسل ، وتشير الألوان المختلفة _إلى أي مدى_ ينمو بسرعة إلى ما لا نهاية:
...
يتكون شريط التمرير هذا من 27 صورة فردية ، حتى مستوى تكبير يزيد عن 14 كوادريليون ، أو `2^54`. إجمالاً ، استغرق الأمر حوالي 45 دقيقة لعرضه على جهاز كمبيوتر محمول حديث. يمكن إنشاء مجموعة Mandelbrot باستخدام معادلة واحدة بسيطة ، `§x_n = x_(n-1)^2 + c` ، ومع ذلك فهي معقدة بشكل لا نهائي وجميلة بشكل مذهل.
...
أثناء تحريك قيمة [{.pill.red} c](target:c) حول مجموعة Mandelbrot ، قد تلاحظ خاصية غريبة:
...
في عام 1985 ، ظهرت مجموعة Mandelbrot على غلاف مجلة _Scientific American_ ، ومنذ ذلك الحين أصبحت واحدة من أكثر الأشكال الرياضية التي يمكن التعرف عليها في العالم. يمكنك العثور عليها على القمصان وفي مقاطع الفيديو الموسيقية وكحافظات للشاشة ، وقد تمت الإشارة إليها في العديد من الكتب والأفلام الشائعة.
